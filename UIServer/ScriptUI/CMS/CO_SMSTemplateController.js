app.controller('CO_SMSTemplateController', function ($scope, $http) {
    $scope.CM00035 = {};
    $scope.PagesCM00035 = [];
    ...
    $scope.toolbarOptions = {
        items: [
                 {
                     type: "button", text: "<span class=\"fa fa-plus-circle\"></span> Set SMS Message",
 ...
 , resizable: true
    ...
    $scope.LoadCM00035 = function (TemplateID) {
        $http.get('/CO_SMSTemplate/GetSingle?KaizenPublicKey=' + sessionStorage.PublicKey + "&TemplateID=" + TemplateID).success(function (data) {
            $scope.CM00035 = data;
        }).finally(function () { $scope.CM00035.Status = 2; });
    };
    $scope.LoadCM00035Page = function (ActionName) {
        removeEntityPage($scope.PagesCM00035);
        ...
        $scope.PagesCM00035.push(Page);
    };
    $scope.AddCM00035 = function () {
        $scope.LoadCM00035Page('Create');
        $scope.Clear();
        $scope.CM00035.Status = 1;
        $scope.LoadCaseTypes();
    };
    $scope.EditCM00035 = function (TemplateID) {
        $scope.LoadCM00035Page('Create');
        $scope.Clear();
        $scope.LoadCM00035(TemplateID);
    };
    $scope.SaveData = function () {
        $http.post('/CO_SMSTemplate/SaveData', { 'CM00035': $scope.CM00035, 'KaizenPublicKey': sessionStorage.PublicKey }).success(function (data) {
            if (data.Status == true) {
                $.bigBox({
                    title: data.Massage,
                ...
                $scope.Cancel();
                $scope.GridRefresh('GridCMS_CM00035');
            }
            else {
                $.bigBox({
                    title: data.Massage,
    ...
    $scope.UpdateData = function () {
        $http.post('/CO_SMSTemplate/UpdateData', { 'CM00035': $scope.CM00035, 'KaizenPublicKey': sessionStorage.PublicKey }).success(function (data) {
            if (data.Status == true) {
                $.bigBox({
                    title: data.Massage,
                    content: data.Description,
    ...
    $scope.Delete = function () {
        $.SmartMessageBox({
            ...
            if (ButtonPressed === "Yes") {
                $http.post('/CO_SMSTemplate/DeleteData', { 'CM00035': $scope.CM00035, 'KaizenPublicKey': sessionStorage.PublicKey }).success(function (data) {
                    if (data.Status == true) {
                        $.bigBox({
                            title: data.Massage,
                            content: data.Description,
    ...
    $scope.CaseTypeChanged_SMSTemplate = function () {
        if (angular.isDefined($scope.SelectedLookUp.SelectedType)) {
            $scope.CM00035.TRXTypeID = $scope.SelectedLookUp.SelectedType;
            $scope.GetCM00074_ByCaseType($scope.CM00035.TRXTypeID);
        }
    };
