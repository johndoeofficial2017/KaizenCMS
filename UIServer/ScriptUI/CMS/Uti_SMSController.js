app.controller('Uti_SMSController', function ($scope, $http) {
    ...
    $scope.toolbarOptions = {
        items: [
                 { 
                  ...
                  {
                      type: "button", text: "<span class=\"fa fa-plus-circle\"></span>Refresh SMS ",
                      attributes: { "class": "btn-primary" },
                      click: function (e) {
                          $scope.$apply(function () {
                              $scope.RefreshSMS();
    ...
    $scope.LoadCM00232Page = function (ActionName) {
        removeEntityPage($scope.PagesCM00232);
        var URIPath = "/CMS/Uti_SMS/" + ActionName + "?KaizenPublicKey=" + sessionStorage.PublicKey;
        var Page = { url: URIPath, ActionName: ActionName };
        $scope.PagesCM00232.push(Page); 
    }
    $scope.DemandTemplateChanged = function () {
        $scope.CM00232.TemplateID = $scope.CM00232.SelectedTemplate.TemplateID;
        $scope.CM00232.TemplateContant = $scope.CM00232.SelectedTemplate.TemplateContant;
    };
    $scope.SaveCaseDemandLetter = function () {
        $scope.CM00233 = [];
        var items = $("#GridCMS_CaseDemandLetterCM00232").data("kendoGrid").dataSource.data();
        for (i = 0; i < items.length; i++) {
            var item = items[i];
            if (item.IsSeleceted) {
                var temp = { CaseRef: item.CaseRef };
                $scope.CM00233.push(temp);
            }
        }
        $scope.CM00232.AddressCode = $scope.SelectedLookUp.DebtorAddressCodeType.AddressCode;
        if ($scope.CM00233.length == 0) {
            alert('cases selection is required');
            return;
        }
        $http.post('/Uti_SMS/SaveCaseDemandLetter', {
            'CM00232': $scope.CM00232,
            'CM00233': $scope.CM00233,
            ...
            .success(function (data) {
                if (data.Status == true) {
                    ...
                    //---------------------
                    $scope.Cancel();
                    $scope.GridRefresh('GridCMS_CM00232');
                }
                else {
                    $.bigBox({
                        title: data.Massage,
    ...
    $scope.CM00232TemplateContant = function () {
    ...
    $scope.RefreshSMS = function () {
        //alert($scope.CM00232.MessageTRXID);
        //alert($scope.MY.CompanyID);
        var url = "http://localhost:7070/message/sendsms";
        var data = { MessageTrxId: $scope.CM00232.MessageTRXID, CompanyID: $scope.MY.CompanyID }
        $.post(url, data).done(function (response) {
            alert(JSON.stringify(response));
            //$scope.dialog.open()
        });
    }
    $scope.AddCM00232 = function () {
        $scope.Clear();
        $scope.CM00232 = {};
        $scope.CM00232.Status = 1;
        $scope.DebtorAddressCodeType();
        $http.get('/Uti_SMS/GetNextTransactionID?KaizenPublicKey=' + sessionStorage.PublicKey).success(function (data) {
            $scope.CM00232.MessageTRXID = data;
        }).finally(function () { });
        $scope.CM00232.SelectedTemplate = {};
        $scope.CM00232TemplateContant();
        $http.get('/CO_SMSTemplate/FillDropDownList?KaizenPublicKey=' + sessionStorage.PublicKey + '&UserName=' + $scope.MY.UserName).success(function (data) {
            $scope.DemandTemplates = data;
        }).finally(function () { });
         $scope.LoadCM00232Page('Create');
    }
    $scope.EditCM00232 = function (MessageTRXID) {
        $scope.DebtorAddressCodeType();
        $http.get('/Uti_SMS/GetSingle?KaizenPublicKey=' + sessionStorage.PublicKey + "&MessageTRXID=" + MessageTRXID).success(function (data) {
            $scope.CM00232 = data;
        }).finally(function () {
            $http.get('/CO_SMSTemplate/FillDropDownList?KaizenPublicKey=' + sessionStorage.PublicKey + '&UserName=' + $scope.MY.UserName).success(function (data) {
                $scope.DemandTemplates = data;
            }).finally(function () {
                $scope.CM00232.SelectedTemplate = $scope.GetSingle($scope.DemandTemplates, "TemplateID", $scope.CM00232.TemplateID);
                $scope.SelectedLookUp.DebtorAddressCodeType = $scope.GetSingle($scope.CM00009List, "AddressCode", $scope.CM00232.AddressCode);
                $scope.LoadCM00232Page('LetterView');
                $scope.CM00232TemplateContant();
            });
        });
    };
