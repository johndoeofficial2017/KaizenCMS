app.controller('Set_Lookup01Controller', function ($scope, $http) {
    $scope.CM00016 = {};
    $scope.PagesCM00016 = [];
    $scope.toolbarOptions = {
        items: [
                 {
                     type: "button", text: "<span class=\"fa fa-plus-circle\"></span> Add Case Lookup",
 ...
 , resizable: true
    ...
    $scope.LoadCM00016 = function (Lookup01) {
        $http.get('/Set_Lookup01/GetSingle?KaizenPublicKey=' + sessionStorage.PublicKey + "&Lookup01=" + Lookup01).success(function (data) {
            $scope.CM00016 = data;
        }).finally(function () { $scope.CM00016.Status = 2; });
    };
    $scope.LoadCM00016Page = function (ActionName) {
        removeEntityPage($scope.PagesCM00016);
        ...
        $scope.PagesCM00016.push(Page);
    };
    $scope.AddCM00016 = function () {
        $scope.LoadCM00016Page('Create');
        $scope.Clear();
        $scope.CM00016.Status = 1;
    };
    $scope.EditCM00016 = function (TemplateID) {
        $scope.LoadCM00016Page('Create');
        $scope.Clear();
        $scope.LoadCM00016(TemplateID);
    };
    $scope.SaveData = function () {
        $http.post('/Set_Lookup01/SaveData', { 'CM00016': $scope.CM00016, 'KaizenPublicKey': sessionStorage.PublicKey }).success(function (data) {
            if (data.Status == true) {
                $.bigBox({
                    title: data.Massage,
                ...
                $scope.Cancel();
                $scope.GridRefresh('GridCMS_CM00016');
            }
            else {
                $.bigBox({
                    title: data.Massage,
    ...
    $scope.UpdateData = function () {
        $http.post('/Set_Lookup01/UpdateData', { 'CM00016': $scope.CM00016, 'KaizenPublicKey': sessionStorage.PublicKey }).success(function (data) {
            if (data.Status == true) {
                $.bigBox({
                    title: data.Massage,
                    content: data.Description,
    ...
    $scope.Delete = function () {
        $.SmartMessageBox({
            ...
            if (ButtonPressed === "Yes") {
                $http.post('/Set_Lookup01/DeleteData', { 'CM00016': $scope.CM00016, 'KaizenPublicKey': sessionStorage.PublicKey }).success(function (data) {
                    if (data.Status == true) {
                        $.bigBox({
                            title: data.Massage,
                            content: data.Description,
