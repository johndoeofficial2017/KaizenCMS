...
{
    public class CPR_DataController : Controller
    {
        // GET: Tools/CPR_Data
        ...
        #region Grid Actions
        private DataSourceResult CaseList([DataSourceRequest]DataSourceRequest request, DataCollection<CPR00100> L)
        {
     ...
     , string FieldID, int FltrOperator, string Searchcritaria)
        {
            Kaizen.Configuration.KaizenSession KaizenUser = UserServices.GetKaizenSession(KaizenPublicKey);
            if (KaizenUser == null) return RedirectToAction("LoginToCompany", "SysUser");
            CPR00100Services invService = new CPR00100Services(KaizenUser);
            DataCollection<CPR00100> L = new DataCollection<CPR00100>();
        ...
        public ActionResult GetDataListGrid([DataSourceRequest]DataSourceRequest request, int viewID, string ViewWhereCondition, string ViewSortCondition, string Searchcritaria, string KaizenPublicKey)
        {
            ...
            CPR00100Services serv = new CPR00100Services(KaizenUser);
            DataCollection<CPR00100> L = new DataCollection<CPR00100>();
            string filters = string.Empty;
...
, string DebtorID, string Searchcritaria, string FieldID = "-1", int FltrOperator = 8)
        ...
        {
            Kaizen.Configuration.KaizenSession KaizenUser = UserServices.GetKaizenSession(KaizenPublicKey);
            CPR00100Services service = new CPR00100Services(KaizenUser);
            CPR00100 o = service.GetSingle(DebtorID);
            return Json(o, JsonRequestBehavior.AllowGet);
        }
        public ActionResult GetRecordTypes(string KaizenPublicKey)
        {
            Kaizen.Configuration.KaizenSession KaizenUser = UserServices.GetKaizenSession(KaizenPublicKey);
            CPR00100Services service = new CPR00100Services(KaizenUser);
            IList<CPR00001> CPR00001List = service.GetRecordTypes();
            return Json(CPR00001List, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult UpdateData(CPR00100 CPR00100, string KaizenPublicKey, bool PhotoChanged, bool SignatureChanged)
        {
            Kaizen.Configuration.KaizenSession KaizenUser = UserServices.GetKaizenSession(KaizenPublicKey);
            if (KaizenUser == null) return RedirectToAction("LoginToCompany", "SysUser");
            int startindex;
            if (PhotoChanged)
            {
                if (!string.IsNullOrEmpty(CPR00100.PhotoExtension))
                {
                    string PhotoPath = @"\\Photo\\DebtorPhotoTemp\\" + CPR00100.PhotoExtension.Trim();
                    startindex = CPR00100.PhotoExtension.LastIndexOf('.');
                    startindex += 1;
                    CPR00100.PhotoExtension = CPR00100.PhotoExtension.Substring(startindex, CPR00100.PhotoExtension.Length - startindex);
                    if (System.IO.File.Exists(Server.MapPath(PhotoPath)))
                    {
                        string Destination = Server.MapPath(@"\\Photo\\DebtorPhoto\\" + CPR00100.DebtorID.Trim() + "." + CPR00100.PhotoExtension);
            ...
            if (SignatureChanged)
            {
                if (!string.IsNullOrEmpty(CPR00100.SignatureExtension))
                {
                    string SignaturePath = @"\\Photo\\DebtorSignatureTemp\\" + CPR00100.SignatureExtension.Trim();
                    startindex = CPR00100.SignatureExtension.LastIndexOf('.');
                    startindex += 1;
                    CPR00100.SignatureExtension = CPR00100.SignatureExtension.Substring(startindex, CPR00100.SignatureExtension.Length - startindex);
                    if (System.IO.File.Exists(Server.MapPath(SignaturePath)))
                    {
                        string Destination = Server.MapPath(@"\\Photo\\DebtorSignature\\" + CPR00100.DebtorID.Trim() + "." + CPR00100.SignatureExtension);
                        if (System.IO.File.Exists(Destination))
                        {
                            System.IO.File.Delete(Destination);
                        }
            ...
            CPR00100Services service = new CPR00100Services(KaizenUser);
            return Json(service.Update(CPR00100), JsonRequestBehavior.AllowGet);
        }
        public ActionResult SaveData(CPR00100 CPR00100, string KaizenPublicKey)
        {
            Kaizen.Configuration.KaizenSession KaizenUser = UserServices.GetKaizenSession(KaizenPublicKey);
            if (KaizenUser == null) return RedirectToAction("LoginToCompany", "SysUser");
            int startindex;
            ...
            CPR00100Services service = new CPR00100Services(KaizenUser);
            return Json(service.AddCPR00100(CPR00100), JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult DeleteData(CPR00100 CPR00100, string KaizenPublicKey)
        {
            Kaizen.Configuration.KaizenSession KaizenUser = UserServices.GetKaizenSession(KaizenPublicKey);
            if (KaizenUser == null) return RedirectToAction("LoginToCompany", "SysUser");
            CPR00100Services service = new CPR00100Services(KaizenUser);
            return Json(service.Delete(CPR00100.DebtorID), JsonRequestBehavior.AllowGet);
        }
