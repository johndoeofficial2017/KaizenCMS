...
(function($) {
    $.widget("ui.chatbox", {
        options: {
            ...
            boxOpened: function(id){
                //Added by Mhisham
            },
            getHistory: function(id,chatElement){
                //Added by Mhisham
            },
            boxManager: {
                ...
                highlightBox: function() {
                    var self = this;
                    self.elem.uiChatboxTitlebar.effect("highlight", {}, 200);
                    self.elem.uiChatbox.effect("bounce", {times: 1}, 200, function() {
        ...
        _create: function() {
            ...
            //History
            uiChatboxViewHistory = (self.uiChatboxViewHistory = $('<a href="#" style="text-align:center;display: block;" class="show_history" data-id="'+self.options.id+'">View Old Messages</a>'))
            .addClass('ui-widget-content ' +
                          'ui-chatbox-content '
                         )
            .click(function(event){
                self.options.getHistory(self.options.id,self.uiChatboxLog);
                self.uiChatboxViewHistory.hide();
            }).appendTo(uiChatbox);
            // content
            ...
            if (!self.options.hidden) {
                uiChatbox.show();
                // console.log('open');
                self.options.boxOpened(self.options.id);
            }
